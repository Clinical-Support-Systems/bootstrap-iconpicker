#!/usr/bin/env node
/*
 * Verification script for Font Awesome 6.7.0 icon list in iconset-fontawesome-6-all.js
 * Compares local array against upstream metadata (fa6-icons.json) restricted to FREE styles.
 */
const fs = require('fs');
const path = require('path');

const META_PATH = path.join(__dirname, 'fa6-icons.json');
...
function buildExpected(meta) {
  ...
  for (const [name, def] of Object.entries(meta)) {
    ...
    if (!Array.isArray(def.free)) continue;
    const mapping = { brands: 'fab', solid: 'fas', regular: 'far' };
    for (const style of def.free) {
...
function extractLocalList() {
  ...
  const versionIdx = js.indexOf("version: '6.7.0'");
  if (versionIdx === -1) throw new Error('Could not locate version 6.7.0 declaration.');
  const slice = js.slice(versionIdx);
  const iconsStart = slice.indexOf('icons:');
  if (iconsStart === -1) throw new Error('Could not locate icons: key after version 6.7.0');
...
function main() {
  ...
  console.log('Font Awesome 6.7.0 Verification Summary');
  ...
  if (failOnMissing && missing.length > 0) {
    console.error(`\nFAIL: ${missing.length} missing Font Awesome 6 free icon(s).`);
    process.exit(1);
  }
}
