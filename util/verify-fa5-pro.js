#!/usr/bin/env node
/*
 * Verification script for Font Awesome 5.15.4 Pro icon list in iconset-fontawesome-5-all.js
...
function extractProList() {
  ...
  const versionIdx = js.indexOf("version: '5.15.4_pro'");
  if (versionIdx === -1) throw new Error('Could not locate version 5.15.4_pro declaration.');
  const slice = js.slice(versionIdx);
  const iconsStart = slice.indexOf('icons:');
  if (iconsStart === -1) throw new Error('Could not locate icons: key after version 5.15.4_pro');
